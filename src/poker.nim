...
proc isStraight(cards: seq[Card]): bool =
  ...
  let v0 = cards[0].rankValue
  let v4 = cards[4].rankValue
...
proc sortedRanks(cards: seq[Card]): seq[Rank] =
  result = @[]
  for c in cards: result.add c.rank
  result.sort(proc(a, b: Rank): int = cmp(rankValue(a), rankValue(b)))
