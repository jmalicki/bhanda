## E2E server wrapper: uses nimhttpd as a library, adds ready-fd signalling.
## Started by e2e_serve launcher with NIMHTTPD_READY_FD set; writes one byte when
## listening so the launcher can return to the shell without polling HTTP.
## Usage: e2e_server -p:PORT DIRECTORY
...
proc main() =
  ...
  when defined(posix):
    let readyFdStr = getEnv("NIMHTTPD_READY_FD")
    if readyFdStr.len > 0:
      try:
